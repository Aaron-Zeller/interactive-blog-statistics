...
async function submitAssessmentScore(phase) {
...
async function submitAssessmentPhase(phase) {
  ...
  const saveResult = await submitAssessmentScore(phase);
  if (!saveResult.ok) {
...
const ASSESSMENT_QUESTIONS = [
  ...
  },
  {
    id: "multiple_once_realistic",
    title: "Is this many positives still realistic?",
    prompt: "Suppose all 55 dots are null features, each tested once at α = 0.05, and 6 dots turn orange. Is that still a realistic outcome from chance alone?",
    defaults: { choice: "" },
    m: 55,
    observedFalse: 6,
    alpha: 0.05,
